...
postContext = dateField "date" "%B %e, %Y" <> websiteContext
...
websiteContext :: Context String
websiteContext = canonicalUrl "url" <> Hakyll.defaultContext

archiveContext :: [Item String] -> Context String
archiveContext posts = listField "posts" postContext (return posts) <> websiteContext
